...
class App(QWidget):
    ...
    def init_ui(self):
        ...
        self.textboxlabel1.setText('UUID (add a short, informative name):')
    ...
    def button2_clicked(self):
        ...
        ephys_experiments = gen_metadata_exp(self.dir_name, self.exp_list)
        gen_metadata(self.dir_name, self.exp_list, ephys_experiments=ephys_experiments,
                     uuid=self.uuid, notes=self.notes)
    ...
    def infobutton_clicked(self):
        ...
        msg.setInformativeText("Please begin by opening a folder locally using the"
                               ...
                               "\n\nIf you run into issues or need S3 credentials, please email(ash@ucsc.edu) "
                               "\n\nHappy uploading!")
...
def gen_metadata(dir_name, exp_list, ephys_experiments, uuid='', notes=''):
    # Open file
    ...
    metadata['ephys_experiments'] = ephys_experiments
...
def gen_metadata_exp(dir_name, exp_list):
    '''
    Loop through each experiment file and return list of experiment dicts
    '''
    exp_metadata = get_metadata_exp_template()
    ephys_experiments = []
    for i, exp in enumerate(exp_list):

        print(f'File# {i} at {exp}')
        ...
        try:
            with h5py.File(exp, "r") as f:
                ...
                exp_path = posixpath.join('original', name, os.path.basename(exp))
                ...
                # Set values
                cur_metadata['name'] = name
                cur_metadata['blocks'][0]['num_frames'] = num_frames
                cur_metadata['blocks'][0]['path'] = exp_path
                ...
                ephys_experiments.append(cur_metadata)
                # Write file
