...
import braingeneers.data.datasets_electrophysiology as de  # import datasets_electrophysiology as de
import numpy as np
import time
import pandas as pd
...
class EphysDataset(Dataset):
    ...
    def __init__(self, batch_uuid, experiment_num, sample_size, attr_csv, start, align='center', bounds='exception', length=-1,
        ...
        """
        self.attr_df = attr_csv  # pd.read_csv(attr_csv)
        self.start = start
    ...
    def __getitem__(self, idx):
        ...
        # Check if only 1 channel is passed
        if str(channels).isnumeric():
        ...
        dataset = de.load_data(de.load_metadata(self.UUID), self.exp_num, self.start+self.offset, self.datalen, self.channels)
        ...
        # if 'left', idx should be on left side of data, sampling forward
            ...
            # get bounds
            left_bound = 0  # self.offset
            right_bound = self.sample_size  # self.offset + self.sample_size
            if self.bounds == 'exception':
                # exception means throw an exception regarding usage
                if left_bound < 0 or right_bound > dataset.shape[1]:
                    raise IndexError('Sample size too large in one direction. Please use valid index/sample size pair.')
